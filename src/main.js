...
function applyImageTransform(img) {
  ...
  // transform は applyImageFrame の逆スケール用 → ここでは触らない
}

// 画像フレーム: <100%=clip-path(parent)、>100%=transform:scale(parent)+逆scale(image)
...
function applyImageFrame() {
  ...
  if (imgNatW && imgNatH && parentW && parentH) {
    ...
    // cover の水平クロップ解消点 × ズーム倍率
    const maxW = Math.max(100, Math.round(imgAR / parentAR * 100 * zoom));
    const maxH = Math.max(100, Math.round(parentAR / imgAR * 100 * zoom));
  ...
  // 編集インジケータをクリア（save前に除去 → innerHTMLに含めない）
  parent.style.filter = '';
  activeImage.style.removeProperty('filter');
  ...
  if (isExpanding) {
    // --- 拡大: parentをtransform:scaleで拡大 + 画像を逆scaleで引き伸ばし打消し ---
    const scaleX = Math.max(wPct, 100) / 100;
    const scaleY = Math.max(hPct, 100) / 100;

    // ★ overflow:visible 必須（画像width>100%をクリップさせない）
