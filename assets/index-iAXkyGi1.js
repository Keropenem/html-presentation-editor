...
`);const r=i.querySelectorAll("section").length?Array.from(i.querySelectorAll("section")):i.querySelectorAll(".slide").length?Array.from(i.querySelectorAll(".slide")):[i.body];c.slides=r.map((l,e)=>({id:`imported-${e}`,content:l===i.body?l.innerHTML:l.outerHTML,css:""})),A(),x(0)}function H(){const n=document.querySelector(".editor-viewport"),o=t.editorContainer;if(!n||!o)return;const i=80,r=Math.min((n.clientWidth-i)/1122.52,(n.clientHeight-i)/793.7);o.style.transform=`scale(${Math.max(r,.1)}) translate(-50%, -50%)`,o.style.left="50%",o.style.top="50%",o.style.position="absolute"}function J(n=1){const o=t.printFrame,i=o.contentWindow.document,r={1:{orientation:"landscape",cols:1,rows:1},4:{orientation:"portrait",cols:2,rows:2},6:{orientation:"portrait",cols:2,rows:3},8:{orientation:"portrait",cols:2,rows:4}},l=r[n]||r[1],e=c.globalStyles.replace(/@page\s*\{[^}]*\}/gi,"").replace(/(?:^|\})\s*(?:html|body)\s*\{[^}]*\}/gi,"}"),a=97,s=68.6,d=3,f=((210-(a*2+d))/2).toFixed(1),E=(a/297).toFixed(6);let v="";if(n===1)v=c.slides.map(S=>`<div class="print-page">${S.content}</div>`).join("");else{const S=[];for(let y=0;y<c.slides.length;y+=n)S.push(c.slides.slice(y,y+n));v=S.map(y=>`<div class="handout-page">${y.map(R=>`<div class="grid-cell"><div class="cell-slide" style="zoom:${E}">${R.content}</div></div>`).join("")}</div>`).join("")}const q=`
    ...
    * { box-sizing: border-box; }
    ...
    ${e}
  `;i.open(),i.write(`<html><head><style>${q}</style></head><body>${v}</body></html>`),i.close(),setTimeout(()=>{o.contentWindow.focus(),o.contentWindow.print()},500)}function K(n){let o=c.globalStyles.replace(/100vw/g,"100%").replace(/100vh/g,"100%").replace(/(^|,)\s*body(?=\s|\{|,)/gi,"$1 .slide-frame").replace(/html(?=\s|\{|,)/gi,".slide-frame");const i=c.slides.map((s,d)=>`<div class="slide-a4-container" id="slide-${d}"><div class="slide-frame">${s.content}</div></div>`).join(`
`);let r="";n&&(r=`
    <div id="ui-layer">
